...
If you wish to contribute to FormBootstrap, please read the [CONTRIBUTING.md](CONTRIBUTING.md) file.

Demonstration / example
-----------------------

Render forms, buttons, alerts with FormBootstrap : see it in action [on-line](http://neilime.github.io/zf2-twb-bundle/demo.html).
...
#### Form : `FormBootstrap\Form\View\Helper\BootstrapForm`

Form helper can be called in a view with the view helper service `form(\Zend\Form\FormInterface $oForm = null, $sFormLayout = \FormBootstrap\Form\View\Helper\BootstrapForm::LAYOUT_HORIZONTAL)` :
```php
<?php
...
```php
<?php
$this->form(null,\FormBootstrap\Form\View\Helper\BootstrapForm::LAYOUT_INLINE)->openTag($oForm);
```
...
* `FormBootstrap\Form\View\Helper::LAYOUT_HORIZONTAL` : horizontal (default)
* `FormBootstrap\Form\View\Helper::LAYOUT_INLINE` : inline

The helper auto add form specific class and `form` role attributes.

#### Button : `FormBootstrap\Form\View\Helper\BootstrapFormButton`
...
* string `position` : (optional) the position of the glyphicon to prepend or append the button content, `\FormBootstrap\Form\View\Helper\BootstrapFormButton::GLYPHICON_PREPEND` (the default) and `\FormBootstrap\Form\View\Helper\BootstrapFormButton::GLYPHICON_APPEND`.
...
#### Checkbox : `FormBootstrap\Form\View\Helper\BootstrapFormCheckbox`
...
#### Form collection : `FormBootstrap\Form\View\Helper\BootstrapFormCollection`
...
#### Form element : `FormBootstrap\Form\View\Helper\BootstrapFormElement`
...
#### Form element errors : `FormBootstrap\Form\View\Helper\BootstrapFormElementErrors`
...
#### Multi-Checkbox : `FormBootstrap\Form\View\Helper\BootstrapFormMultiCheckbox`
...
#### Radio : `FormBootstrap\Form\View\Helper\BootstrapFormRadio`
...
#### Form row : `FormBootstrap\Form\View\Helper\BootstrapFormRow`
...
#### Static : `FormBootstrap\Form\View\Helper\BootstrapFormStatic`
...
#### Button group : `FormBootstrap\View\Helper\BootstrapButtonGroup`
...
```php
<?php
$this->dropdown(array('Item #1',\FormBootstrap\View\Helper\BootstrapDropDown::TYPE_ITEM_DIVIDER,'Item #2'));
```
 ...
 * `items` (array) : list of items, should contains :
  * `scalar` :
   * `\FormBootstrap\View\Helper\BootstrapDropDown::TYPE_ITEM_DIVIDER` : display a divider
   * `text` : display a text (translated) into a link (anchor attribute is the same as content)
  * `array` : the item options :
   * `type` (string) : the type of item
    * `\FormBootstrap\View\Helper\BootstrapDropDown::TYPE_ITEM_HEADER` : render an item as header
    It needs the following option :
     * `label` (scalar) content text  (translated) of the item
      * `\FormBootstrap\View\Helper\BootstrapDropDown::TYPE_ITEM_DIVIDER` : render a divider
      * `\FormBootstrap\View\Helper\BootstrapDropDown::TYPE_ITEM_LINK` : render an item as a link `<a ...>`
    It needs the following option :
     * `label` (scalar) content text  (translated) of the item
...
#### StaticElement : `FormBootstrap\Form\Element\StaticElement`
...
```php
<?php
$this->formStatic(new \FormBootstrap\Form\Element\StaticElement());
```
