...
class BootstrapFormRow extends FormRow
{
    ...
    public function __construct(BootstrapForm $form)
    {
        $this->form = $form;
        $this->formLayout = $form->getFormLayout();
    }
    ...
    protected function renderElement(ElementInterface $oElement)
    {
        ...
        if ($sLabelContent = $this->renderLabel($oElement)) {
            ...
            if (($oElement instanceof Button) or ($oElement instanceof Submit)) {
                ...
                if ($oElement->getOption('validation-state') || $oElement->getMessages()) {
                    if (!isset($aLabelAttributes['class']) || empty($aLabelAttributes['class'])) {
                ...
                switch ($sLayout) {
                    ...
                    case BootstrapForm::LAYOUT_INLINE:
                    ...
                    case BootstrapForm::LAYOUT_HORIZONTAL:
                        if ($sElementType !== 'checkbox') {
                            if (!isset($aLabelAttributes['class']) || empty($aLabelAttributes['class'])) {
                                $aLabelAttributes['class'] = "control-label {$this->labelClassSize}";
                            } else {
                                if (!preg_match('/(\s|^)control-label(\s|$)/', $aLabelAttributes['class'])) {
                                    $aLabelAttributes['class'] = trim($aLabelAttributes['class'] . ' control-label');
                                }
                                if (!preg_match("/(\s|^){$this->labelClassSize}(\s|$)/", $aLabelAttributes['class'])) {
                                    $aLabelAttributes['class'] = trim($aLabelAttributes['class'] . " {$this->labelClassSize}");
                                }
                            }
                        }
                        break;
        ...
        switch ($sLayout) {
            ...
            case BootstrapForm::LAYOUT_HORIZONTAL:
                ...
                if (empty($sClass)) {
                    $sClass = " " . $this->fieldClassSize;
                } else {
                    if (!preg_match("/(\s|^){$this->fieldClassSize}(\s|$)/", $sClass)) {
                        $sClass = trim($sClass . " {$this->fieldClassSize}");
                    }
                }
    ...
    protected function getElementLayout(ElementInterface $element)
    {

        if ($element->getOption('twb-layout')) {
            return $element->getOption('twb-layout');
        } else {
